...
CUDA_DEVICES=1
stage=0
datalst=/exdata/HOME/snie/code/asvspoof/speaker_asvspoof/dataset_min/speech.lst
datadir=/exdata/HOME/snie/code/asvspoof/speaker_asvspoof/dataset_min/speaker/
...
if [ $stage -le 1 ]; then 
  ...
  python genASR_train_dev_test_new.py $datadir/speech/all 8.0:1.0:1.0 $datadir/speech/ || exit 1;
fi

# Prepare train, dev and test for noise
if [ $stage -le 2 ]; then 
  echo "step-3: prepare train, dev and test for noise"
  python gen_train_dev_test.py ${datadir}/noise/all/interference/interference.scp 8.0:1.0:1.0 ${datadir}/noise/interference/ || exit 1;

  python gen_train_dev_test.py ${datadir}/noise/all/diffuse_noise/diffuse_noise.scp 8.0:1.0:1.0 ${datadir}/noise/diffuse_noise/ || exit 1;
fi
